...
const movingImg = (evt) => {
  ...
  $(".box4-left").css({ left: -200 - x / 50, top: 70 - y / 30 });
  $(".box4-right").css({ right: -50 + x / 50, bottom: -50 - y / 50 });
  // 살짝 덜 버벅이는 느낌
...
$(window).on("scroll", function () {
  ...
  // $(".global-nav li a").eq(idx).addClass("active");
  ...
  $(this).on("wheel", function (evt) {
    ...
    if (delta > 0) {
      if (nextBox) {
        ...
        currentTop = this.offsetTop; // 다음 박스의 값이 없는 경우 현재 박스의 높이를 반환하기 때문에 멈춤
      }
      // 휠을 올렸을 때
    } else {
      if (prevBox) {
        ...
        // currentTop = 0; 첫번째 박스에 도달하면 높이값 0으로 반환하기 때문에 첫번째 박스에서 멈춤
        return;
      }
    }
    window.scrollTo({
